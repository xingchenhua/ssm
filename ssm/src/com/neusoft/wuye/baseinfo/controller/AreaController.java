...
public class AreaController {
   ...
   public AreaModel get(@RequestParam int areaNo) throws Exception
   {
	   return as.get(areaNo);
   }
   //取得所有小区列表，不分页
   ...
   public ResultInfo<AreaModel> getListByAllWithPage(@RequestParam(required=false,defaultValue="10") int rows,@RequestParam(required=false,defaultValue="1") int page) throws Exception
   {
	...
	public ResultMessage checkCanDelete(@RequestParam int areaNo) throws Exception
	{
		ResultMessage result=new ResultMessage();
		if(as.checkCanDelete(areaNo)){
			result.setResult("Y");
	...
	{
		ServletContext application=session.getServletContext();
		String sourcepath=application.getRealPath("/excelexport/areaexport.xlsx");
